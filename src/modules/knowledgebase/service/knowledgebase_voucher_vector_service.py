...
class KnowledgeBaseVoucherVectorService:
    """
    知识库向量服务（团购券分类）。
    """

    def __init__(self) -> None:
        index_url: str = f"{app_config.knowledgebase_index_name}/voucher"
        self.vector_service: VectorService = VectorService(index_url)
    ...
    async def vectorize_and_store(self, kb_id: int, kb_name: str, content: str) -> None:
        ...
        try:
            ...
            for i in range(batch_count):
                ...
                logger.debug(
                    "处理第 %s/%s 批: chunks %s-%s, 第一篇文档长度=%s",
                    i + 1,
                    batch_count,
                    start + 1,
                    end,
                    len(batch[0].page_content),
                )
